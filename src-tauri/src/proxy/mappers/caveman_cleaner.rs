...
// Spaces cleanup
static SPACE_PUNCT_RE: Lazy<Regex> = Lazy::new(|| {
...
impl CavemanCleaner {
    ...
    pub fn clean(text: &str) -> String {
        ...
        let placeholder_text = PROTECTED_RE.replace_all(text, |caps: &regex::Captures| {
        ...
        let placeholder_text_protected = make_sure_re.replace_all(&placeholder_text, |caps: &regex::Captures| {
        ...
        compressed_text = restore_make_sure_re.replace_all(&compressed_text, |caps: &regex::Captures| {
            let verb = caps.get(1).unwrap().as_str().to_string();
            format!("{} sure", verb)
        }).to_string();
